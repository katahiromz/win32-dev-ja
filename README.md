...
ビットマップのイメージをファイルに保存できるようにしよう。
しかし、Win32 APIでは、ビットマップをファイルに保存するという簡単で便利な関数は存在しない。
そこで次のコードを参考にする。

- [katahiromz/SaveBitmapToFile](https://github.com/katahiromz/SaveBitmapToFile)

次のような`bitmap.cpp`ファイルを追加する。
...
そして、`CMakeLists.txt`の`add_executable`に`bitmap.cpp`を追記する。

さらに`paint.h`に次の関数宣言を追加する。
...
これで`LoadBitmapFromFile`/`SaveBitmapToFile`関数を呼べばビットマップを保存できるようになった。
